...
\Correct{However, recent research has indicated that the human genome harbours an excess of rare, functional variants, which may entail deleterious effects, as a result of recent, explosive growth and weak purifying selection} \citep[\eg, see][]{Kryukov:2007ec,Marth:2011bz, Coventry:2010cqa,Keinan:2012kl,Tennessen:2012ck}.
...
Statistical models in population genetics may therefore resort to the concept of an effective population size, denoted by \Ne, to substitute the census population size, $N$.

\Addition{The concept was introduced by} \citet{Wright:1931ub} \Addition{and further developed by many others;} \eg \citet{crow1970introduction}.
The value of \Ne is defined as the size of an ``ideal'' (Wright-Fisher) population \citep{fisher1930genetical,Wright:1931ub}, which would result in the same value of a given genetic property of interest.
Several concepts of \Ne have been developed; for example the inbreeding effective size, variance effective size, and the coalescent effective size \citep[\eg, see]{Wang:2005wy}.



The effective population size corresponds to the size of an ``ideal'' (Wright-Fisher) population, which typically is much smaller than the census population size.

The value of \Ne is generally difficult to estimate,

the stochastic nature of processes and factors such as genetic drift, inbreeding, sex-ratio, number of breeding individuals, non-random mating, reproductive success (to name a few)
which affect the genetic variation within a population.

in which mating is random, there is no selection, there is no population structure, there is no mutation and generations are non-overlapping.
...
However, because past population sizes are typically difficult to assess,


\Addition{For example,} \Ne can be estimated from the genetic diversity observed in a population.
...
\begin{equation*}
	T_\text{total} ~=~ i T_i ~+~ (i-1) T_{i-1} ~+~ (i-2) T_{i-2} ~+~ \ldots ~+~ 2 T_2 \CorrectLabel
\end{equation*}
