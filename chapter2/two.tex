...
Although data are not accessible publicly, an online service has been provided for imputation and phasing from the internally stored reference dataset\footnote{HRC: \url{http://www.haplotype-reference-consortium.org} \accessed{2017}{02}{05}}.
...
Multiple reference panels were derived from the \glsentryfull{1kg} Phase~\rom{1} dataset, which comprises both low-coverage \glsentrylong{wgs} and \glsentrylong{wes} data of \n{1092} individuals from \n{14}~populations of European, East-Asian, African, and admixed American ancestries.\footnote{Note that I completed work on the \emph{meta-imputation} project prior to the release of \gls{1kg} Phase~\rom{3} \mbox{\citep{Auton:2015gk}}.}
...
The dataset is composed of data obtained on several platforms; low-coverage \glsentrylong{wgs} ($\sim5\times$), high-coverage \glsentrylong{wes} ($\sim82\times$), and genotyping data using the \emph{Illumina Omni2.5 Array}.
...
\Cpref{fig:sfs_got2d} shows the \gls{sfs} of variants captured in the \gls{got2d} dataset, highlighting the discrepancy between the frequency distribution observed at all captured variants and those obtained through genotyping only.
...
These metrics were computed using \texttt{QCTOOL},\footnote{\texttt{QCTOOL}: \url{http://www.well.ox.ac.uk/~gav/qctool} \accessed{2016}{12}{02}} which was performed on both directly imputed and meta-imputed datasets.
...
First, I report the impact of \glsentryfull{qc} to characterise the different meta-imputation settings by the number of retained sites.
