...
However, to facilitate a closer comparison to the results obtained on the simulated dataset, \Correct{which used a variable recombination rate as provided by the Built~37 \gls{hapmap} Phase~\rom{2} genetic map for chromosome~20} (see \ccref{sec:simdata}), the following results focus on chromosome~20 only.
...
\Correct{The main observation from applying the \gls{fgt} and \gls{dgt} to real data is that the detected shared haplotype segments appear to be shorter than suggested by the previous analysis on simulated data.
...
The presence of error in real data cannot be ruled out; \Correct{in particular, given the known error rates in current sequencing and genotyping technologies,
