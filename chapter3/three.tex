...
Hence, because} \fk{2} \Correct{haplotypes are likely to be longer compared to haplotypes tagged by higher-frequency alleles, phasing errors are more likely to lead to an underestimation of shared haplotype lengths.}
...
\Correct{Note that the method requires haplotype data and cannot be executed with genotype information alone.
...
on average, found \dec{1.980775} and \dec{1.990609} per haplotype pair using true haplotypes \ref{app:fgt_h} and phased haplotypes \ref{app:fgt_p}, respectively.
\dec{1.137447} per haplotype pair in the set of unique true IBD segments

The majority of inferred IBD segments were relatively short in length;
for example, \Percent{1.424919} and \Percent{1.420697} of the detected segments were longer than 1~\gls{Mb} in \ref{app:fgt_h} and \ref{app:fgt_p}, respectively,
compared to \Percent{14.79594} in the set of unique true segments.

\n{976503} (\Percent{32.53868}) were matched in \ref{app:fgt_h}
\n{236644} (\Percent{7.885366}) in \ref{app:fgt_p}
...
\Correct{Note that the purpose of this analysis is to evaluate whether \texttt{Refined\,IBD} could be used as a method to detect recombination breakpoints.
Here, it is not directly possible, for example, to make statements about IBD segments falsely identified by \texttt{Refined\,IBD}, as these would be among the segments removed in the matching process.}
...
\Correct{These results suggested} that the lengths of inferred breakpoint intervals are likely to be shorter than the underlying haplotype region shared by descent.
