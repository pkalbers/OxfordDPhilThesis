...
%; \ie the frequency of an allele is assumed to be a proxy to its age
...
%
\input{chapter3/fig/info_testfail_example}
%
...
This was also reflected in the measured magnitude of error (\gls{rmsle}), which was \dec{0.39983}, \dec{0.43407}, and \dec{0.56940} in \ref{app:fgt_h}, \ref{app:fgt_p}, and \ref{app:dgt}, respectively.
...
For example, for \fk{2}~variants, $r^2$ was \dec{0.998}, \dec{0.895}, and \dec{0.995} in \ref{app:fgt_h}, \ref{app:fgt_p}, and \ref{app:dgt}, respectively,
...
% \CorrectNote{Section rewritten to include multiple corrections and additional results.}
...
%
\input{chapter3/fig/beagle_overlap}
%
...
The IBD detection methodology developed in this chapter (\gls{fgt} and \gls{dgt}) was applied to the final release dataset of the \glsentrylong{1kg} Phase~\rom{3} \citep{GenomesProjectConsortium:2012co,Auton:2015gk}, which included ${N=\num{2504}}$ individuals.
...
However, to facilitate a closer comparison to the results obtained on the simulated dataset, which used a variable recombination rate as provided by the Built~37 \gls{hapmap} Phase~\rom{2} genetic map for chromosome~20 (see \ccref{sec:simdata}), the following results focus on chromosome~20 only.
A summary of the IBD detection results for chromosomes \mbox{1--22} is given in \cpref{tab:stat_ibd_1kg}.
...
As was seen in the analysis of simulated data, the \gls{dgt} generally is more likely to overestimate breakpoint distance, leading to the discovery of longer intervals.
...
%
\input{chapter3/fig/ibd_example_1000G}
%
...
The example shown in \cpref{fig:ibd_example_1000G} may support this notion.
...
This pattern can be compared to \cref{fig:full_ibd_example}{a} (\pref{fig:full_ibd_example}), which showed a similar example from the simulated dataset.
...
The IBD results obtained from analysis of the \gls{1kg} dataset suggested that the \gls{fgt} was similarly affected by phasing error as seen in the simulation analysis.
