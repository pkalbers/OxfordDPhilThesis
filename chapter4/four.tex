...
\Cref{eq:errormatrix,eq:errfrqprob} provide a generic framework for the error rate of genotypes and the calculation of genotype frequencies after error.
\N{2} error models are presented below which provide formulations for the calculation of model parameters $\varepsilon_{ij}$.
...
Error functions for both models are given in \cpref{tab:errormodel}; note that these are arranged as in error matrix $\mathcal{E}$ in \cref{eq:errormatrix}.
To illustrate the expectations arising from these models, \cpref{fig:error_models} shows the expected proportion of a true genotype observed as the same or another genotype, at different, nominal error rates, for both the genotype and allele-based models.
...
Thus, inheritance constraints were most informative for the \n{2} parents, labelled \textsf{NA12877} and \textsf{NA12878} (Coriell~ID), which were additionally sequenced to ${200\times}$ depth, and for which high-confidence variant calls were made available.
...
\Citet{Eberle:2016ki} identified high-confidence regions in the 13 individual call sets (\textsf{NA12877}, \textsf{NA12878}, and their 11 children) by collating sites that were called as being homozygous for the reference allele and monomorphic in the sample.
...
Together, the total number of matched genotypes was \dec{4.233817}~million in \emph{Omni2.5} and \dec{1.715609}~million in \emph{Affy6.0}, and where \dec{1.361437}~million ($\approx$32\%) and \dec{0.793548}~million ($\approx$46\%) of true genotypes were assumed as being homozygous for the reference allele, respectively.
...
As formulated in \ctref{eq:errfrqprob}, the observed genotype frequency is a function of the true allele frequency and error rates of genotypes.
...
Using \ctref{eq:empexperr}, the proportions of observed genotypes were calculated in each bin, to obtain error rate expectations across the frequency spectrum.
...
For each dataset, the resulting error rate distributions are shown in \cpref{fig:generrprop}.
...
Diploid individuals were formed by pairing haplotypes.
...
Then, for each class~$j$, indexed pairs were randomly drawn and assigned to each of the \n{3} observed genotype classes, in proportions equal to empirical error rates, as determined for the given allele frequency of the currently selected site.
...
Recall that, for example, the empirically determined proportions of the true $g_0$ class per observed $\tilde{g}_2$ were likely to be inflated at higher allele frequencies; as discussed in \cpref{sec:emp_gen_err_prop}.
...
To illustrate the problem, consider the example given in \cpref{fig:full_ibd_error}, which highlights the effect of error for breakpoint detection using the \gls{fgt} and \gls{dgt}.
...
The probabilistic \texttt{Refined\,IBD} method implemented in \texttt{Beagle} version~4.1 \citep{Browning:2013eh} was used for IBD detection in data after the inclusion of error.\footnote{Beagle 4.1: \url{https://faculty.washington.edu/browning/beagle/beagle.html} \accessed{2016}{11}{22}}
...
Conversely, in cases where a rare allele was falsely called or typed, it can be expected that the actual relationship between the haplotypes at that position is relatively old, such that the interval detected is likely to be relatively short.
...
The expected age of an allele, measured in scaled time units and denoted by $\tau_k$, can be estimated directly from its frequency; as already presented in \cpref{sec:alleleage}.
...
For example, under the infinite sites model, it is expected that genotypes $g_{0,2}$ and $g_{2,0}$ cannot be observed if they share a haplotype by descent.
...
At \fk{2} alleles, median length was
...
The distribution of IBD length by focal allele frequency is shown in \cpref{fig:length_hmm}.
% \Delete{, which for comparison also includes the results obtained for the FGT and DGT after error}.
...
%
\input{chapter4/tab/1kg20_pop_len}
%
...
For example, as given in \ctref{eq:genpairibd}, the expected probability to observe genotype pair $g_{02}$ or $g_{20}$ in \emph{ibd} would be equal to zero.
...
Another consideration is that focal allele frequency may not be an ideal indicator for allele age or, in particular, \gls{tmrca}.
...
In presence of data error, not all observed shared alleles correctly identify a recently co-inherited shared haplotype, which is particularly problematic towards lower allele frequencies; for example, see \cpref{fig:fk_error}.
