...
The value of the expected age is nonetheless useful to arrive at approximate transition probabilities in this \gls{hmm}.}
...
The expected emission probability distributions for each possible genotype pair in \emph{non} and \emph{ibd} is shown in \cpref{fig:exp_genpair_frq}.
...
Recall \Correct{that} there are multiple focal alleles that may sit on the same underlying shared haplotype, such that the resulting set of segments inferred from all selected target sites may result in duplicate segments.
...
In comparison, the number of unique segments in the set of true IBD segments, those determined from simulation records on the same targets, was \dec{2.720789}~million (\SI{27.59894}{\percent}).
The proportion of breakpoints overestimated was similar \Correct{for} both datasets; \SI{55.71575}{\percent} before error and \SI{54.09442}{\percent} after error.
...
The distribution of IBD length is shown in \cpref{fig:length_hmm}\Delete{, which for comparison also includes the results obtained for the FGT and DGT after error}.
...
These results are shown in \cpref{fig:1KG_chr20_length_hmm}, which for comparison also includes the previous results obtained for the \gls{fgt} and \gls{dgt} on the same set of retained target sites (duplicate segments were again removed).
