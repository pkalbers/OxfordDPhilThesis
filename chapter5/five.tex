...
\Correct{The approach is inspired by \emph{composite likelihood} methods, which} recently have gained in popularity for various applications in genetic research.
...
\Correct{Given a sample of haplotypes, where more than one haplotypes carry the focal allele, it assumed that they co-inherited the allele from that one chromosome in which the mutation occurred at some point in the past.
According to coalescent theory, any two haplotypes that share the allele have coalesced more recently than the time of the focal mutation event.
Likewise, the time of coalescence between one haplotype carrying the allele and one haplotype not carrying the allele is expected to be further back in time.}
...
Thus, the number of mutations which segregate in \n{2} haplotypes, as well as the genetic length of the shared haplotype region can be used to infer the \gls{tmrca} between a given pair haplotypes.
...
\Addition{Each clock model is used to compute a posterior density of the \gls{tmrca} over a coalescent time prior distribution.
...
To put this approach into practise, posteriors are combined in a fashion similar to existing composite likelihood methods, which may therefore be described as a \emph{composite posterior} method.
However, the method is more correctly described as an \emph{ad-hoc} analysis.
This is explained in more detail in \cpref{sec:comp_post_method}.}
...
Considering \cref{eq:rec_prob}, the likelihood function for $\tau$ can be written as
...
At each site, a maximum of \n{100} concordant and discordant pairs was selected at random, yielding \n{362661} pairwise analyses in total.
