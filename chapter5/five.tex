...
\end{equation*}
\Correct{which was already given in \cpref{eq:coal_2_dont}.}
...
The number of mutational differences observed along the segment in the pair of haplotypes is denoted by $s$, which represents the sum of \gls{iid} mutation events over the region of length $D$.
...
\begin{equation}
	P(S = s \mid \theta,D,\tau) ~=~ \frac{(\theta D \tau)^s}{s!} \euler{-\theta D \tau}
\end{equation}
...
\Addition{
Note that both \cref{eq:mut_like,,eq:mut_post} can be expressed in terms of the
}
