...
The average rate seen $\leq100$ generations was taken for \emph{ibd}, and
...
Rate of correctly observing allele pairs (concordant: 11, discordant: 01), measured by comparing alleles before and after including empirical error rates (obtained on 1000 Genomes data), in each possible concordant/discordant pair of haplotypes, at focal alleles at a certain frequency.
...
It would be beneficial to select those haplotypes from $X_d$ that are the nearest neighbours to the sub-tree spanned by by the lineages leading to $X_c$.
...
The implementation of this algorithm substantially accuracy and computation time due to retaining a smaller number of pairs.
...
Note that a nearest neighbour approach could also be applied to select concordant pairs, but which was not done here, as it is assumed that random sampling is sufficient to retain pairs that indicate coalescent events closer to the time of the focal mutation event.
...
Misclassification of genotypes or alleles is expected to adversely affect the estimation when the mutation clock or the combined clock model is used.
False positives (wrongly called or typed alleles) may artificially inflate the number of pairwise differences seen along the inferred shared haplotype region.
...
Thereby, allelic differences are restricted to the mutations that have occurred more recently than the focal mutation event, within the sub-tree deriving from that ancestral lineage.
This rule derives from the infinite sites model, where it is assumed that no back-mutations or recurrent mutations can be observed in the data.

The implementation of this rule substantially improved estimation accuracy in presence of data error,
and did not affect accuracy when data was error-free (simulated haplotypes).
Note that the results shown below were obtained using this restriction.

A similar rule could be implemented for discordant pairs.
However, the length of share haplotype segments is expected to be (exponentially) shorter, reducing the possibility to encounter misclassified alleles.
Also, overestimation of coalescent time of discordant pairs (using the mutation clock or combined clock model) is less problematic for the subsequent estimation of allele age.
...
%
\subsection{Comparison to the Pairwise Sequentially Markovian Coalescent (PSMC)}\label{sec:psmc_eval}
%
...
Note that the empirical emission model obtained from data before error (dataset $\mathcal{D}_B$) was used for shared haplotype inference in \texttt{rvage}.
...
\num{50000} sites
Note that all sites at which allele count was greater than 2 were considered.

\dec{2.370018}~million concordant pairs
\dec{5.204300}~million discordant pairs

%
\subsubsection{Inferred T\textsubscript{MRCA} by population}
%

looking at concordant pairs only
of which \dec{2.357372}~million were retained after removing boundary cases.

The average ratio of genetic and physical length was
\SI{1.679922}{\centi\morgan\per\mega\basepair} ($\pm\dec{0.002085}$~SE).
Also, for discordant pairs, the ratio was
\SI{1.625225}{\centi\morgan\per\mega\basepair} ($\pm\dec{0.002149}$~SE).


The proportion of alleles shared within and between populations is indicated.
...
%
\subsubsection{Allele age by population}
%

using the relaxed nearest neighbour approach to select discordant pairs

alleles shared only within populations
retained \n{31906} sites

Median age:
C         M        R
AFR 865.0830 1276.4652 252.3746
AMR 364.4270  639.2214  59.3160
EAS 397.3336  712.1676  59.3160
EUR 364.4270  653.1868  66.0850
SAS 414.8850  743.6260  64.6720
