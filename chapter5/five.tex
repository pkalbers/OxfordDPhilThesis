...
An example is given in \cpref{fig:age_example}, showing the \gls{ccf} posterior distributions for concordant and discordant pairs, as well as the maximised composite posterior distribution.
...
The ``composite posterior'' given in \ctref{eq:comp_post} has a similar form as the above, but where ${\Lambda_k(t)}$ is proportional to the product of posteriors and, thus, cannot be regarded as a composite likelihood.
...
The allele age estimation method presented in this chapter relies on inference of the haplotype region shared by descent between \n{2} chromosomes relative to a target site.
...
%, which were randomly paired to form a sample of ${N = \num{1000}}$ diploid individuals.
The length of the simulated region was \SI{100}{\mega\basepair} (\SI{100}{\centi\morgan}), resulting in \n{326335}~variant sites.



The breakpoints of shared haplotype intervals were identified on basis of the observed variant sites in the sample, such that the resulting true IBD segment defined the smallest interval detectable from available data.
