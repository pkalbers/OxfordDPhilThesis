...
Hence, the \gls{pdf} of a recombination breakpoint observed at distance $d$ is
\begin{equation}\label{eq:recprob_brk}
	P(D=d\mid\psi,\tau)~=~2\psi\tau \,\euler{-2\psi\tau d}\text{.}
\end{equation}
However, when the end of the chromosomal sequence was reached without observing a breakpoint (boundary case), it follows from the Exponential distribution that
...
\begin{equation}\label{eq:rec_like_x}
	\mathcal{L}(\tau \mid \psi, d)~\propto~\tau^b \,\euler{-2\psi d \tau}
\end{equation}
...
\begin{equation}\label{eq:rec_like}
	\mathcal{L}(\tau \mid \psi, h)~\propto~\tau^{b_L + b_R} \,\euler{-2\psi h \tau}
\end{equation}
...
\begin{split}
	p(\tau \mid \psi, h)
	&~\propto~
	\mathcal{L}(\tau \mid \psi, h)~\times~\pi(\tau) \\
	&~\propto~
...
\begin{equation}
	p(\tau \mid \psi, h)~=~\operatorname{Gamma}(\tau\mid b_L + b_R +1, 2\psi h +1)\text{.}
\end{equation}
