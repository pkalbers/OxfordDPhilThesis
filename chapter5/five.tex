...
\Correct{The method is inspired by \emph{composite likelihood} approaches, which} recently have gained in popularity for various applications in genetic research.
...
It follows that the allele is expected to be younger than the \gls{mrca} of that \n{1} individual and any of the \Correct{non-carrier} individuals in the contemporary population.
...
The derivation of the coalescent prior \Correct{follows from the results given in \cpref{sec:coalescent}, but is briefly reiterated below.}
...
\end{equation*}
\Correct{which was already given in \ctref{eq:coal_2_dont}.}
...
Given the time of the \gls{mrca}, mutation events are Poisson distributed, as each mutation represents an independent Bernoulli trial over a large number of sites, where each site has a small probability of mutation.
...
Thus, the probability of observing a discrete number $S$ over distance $D$ and time $\tau$ is modelled as a Poisson process, such that ${S \sim \operatorname{Pois}(\theta D \tau)}$, for which the \Correct{\gls{pmf}} is defined as
\begin{equation}
...
The likelihood function for the time parameter~$\tau$ is proportional to the joint \Correct{\gls{pmf}}, but requires only those terms that involve $\tau$ and where constant terms can be dropped, such that
...
\texttt{PSMC} uses a \gls{hmm} to infer the \gls{tmrca} at a locus from the observed sequence of heterozygote genotypes (phased haplotype pairs).
